...
public class loginController {
    ...
    public String authenticateUser(@RequestParam String username, @RequestParam String password, Model model) {
        try {
            ...
            ResponseEntity<String> authResponse = restTemplate.exchange(authUrl, HttpMethod.POST, requestEntity,
                    String.class);
    ...
    public String dashboaredController(Model model) {
        try {
            ...
            RestTemplate restTemplate = new RestTemplate();
            ResponseEntity<customerDataWithuuid[]> response = restTemplate.exchange(createUrl + "?cmd=" + cmd,
                    HttpMethod.GET, requestEntity, customerDataWithuuid[].class);
    ...
    public String createCustomer(@ModelAttribute CustomerData customerData,
            @RequestParam String token, @RequestParam("user-id") String id,
            Model model) {
        if (id.trim().isEmpty()) {
            ...
            try {
                if (customerData.getFirst_name().trim().isEmpty() || customerData.getLast_name().trim().isEmpty()) {
                    ...
                    RestTemplate restTemplate = new RestTemplate();
                    ResponseEntity<String> response = restTemplate.exchange(createUrl + "?cmd=" + cmd, HttpMethod.POST,
            ...
            try {
                System.out.println("running update class");
                ...
                requestData.put("email", customerData.getEmail());
                requestData.put("phone", customerData.getPhone());
                HttpEntity<String> requestEntity1 = new HttpEntity<>(requestData.toString(),headers);

                RestTemplate restTemplate = new RestTemplate();
                ResponseEntity<String> response = restTemplate.exchange(createUrl + "?cmd=" + cmd + "&uuid=" + uuid,
